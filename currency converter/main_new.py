...
class GUI(Tk):  # CLASS GUI
    ...
    def firstpage(self):
        ...
        # using PIL
        self.image = Image.open('background4.jpeg')
        self.image = self.image.resize((1000, 800), Image.ANTIALIAS)
    ...
    def app_gui(self):
        ...
        # open
        image = Image.open('background4.jpeg')
        # resize
    ...
    def save(self):  # mysql queries to insert data...
        if self.db_variable.get() == 1:
            try:
                ...
                # saving it in realtime database also...
                self.fyrebase = firebase.FirebaseApplication("https://currency-converter-test.firebaseio.com/", None)
                data = {
                    "DATE": f"{self.date}",
                    "FROM_CURRENCY": f"{self.from_.get()}",
                    "TO_CURRENCY": f"{self.to.get()}",
                    "AMOUNT": f"{self.amount.get()}",
                    "RESULT": f"{self.result}"
                }
                self.fyrebase.post("https://currency-converter-test.firebaseio.com/counts", data)
            except:
    ...
    def showdata(self):
        if self.db_variable.get() == 1:
            ...
            TextArea.insert(END, "(  DATE,   FROM, TO, AMOUNT, RES)\n\n")
            for data in self.mydata:
                TextArea.insert(END, f"{data}\n")
            self.cursor.close()
        else:
            self.fyrebase = firebase.FirebaseApplication("https://currency-converter-test.firebaseio.com/", None)
            self.counts_result = self.fyrebase.get("https://currency-converter-test.firebaseio.com/counts", '')
            self.count_table_rows = self.counts_result.values()
            #print(self.count_table_rows)
            root = Tk()
            root.geometry("300x300")
            root.title("GUI-SHOWDATA")
            root.wm_iconbitmap("logo.ico")
            ...
            for row in self.count_table_rows:
                TextArea.insert(END, f"({row['DATE']}, {row['FROM_CURRENCY']}, {row['TO_CURRENCY']}, {row['AMOUNT']},"
                                     f" {row['RESULT']})\n")
